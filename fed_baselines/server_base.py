...
class FedServer(object):
    ...
    def agg(self):
        ...
        for i, name in enumerate(self.selected_clients):
            ...
            for key in self.client_state[name]:
                if i == 0:  # initialize the dict for the first client
    ...
    def rec(self, name, state_dict, n_data, loss):
        ...
        self.client_n_data[name] = {}  # initialize the dict for the client k

        self.client_state[name].update(state_dict)
        self.client_n_data[name] = n_data
        
        self.client_loss[name] = {}  # initialize the dict for the client k
...
if __name__ == '__main__':
    server = FedServer(client_list=[1, 2, 3, 4, 5], dataset_id=0, model_name='cnn')
    server.select_clients(connection_ratio=1)